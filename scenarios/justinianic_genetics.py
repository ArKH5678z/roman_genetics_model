...
# Justinianic Plague — 541 CE
# 541 CE = generation 42
START_YEAR = 541
START_GEN  = 42
DURATION   = 3      # longer — bubonic plague persisted in waves
DAYS       = GENERATIONS

PLAGUE_EVENTS = {
    'Eastern_Med':         [(START_GEN, 0.09, DURATION)],
    'Italian_Central_Med': [(START_GEN, 0.08, DURATION)],
    'Western_European':    [(START_GEN, 0.07, DURATION)]
}
...
def run_justinianic(use_pid=True, migration_rate=0.001):
    """
    Run Justinianic plague simulation across three subpopulations.
...
if __name__ == "__main__":
    print("Justinianic Plague Simulation (541 CE)")
    print("=" * 45)

    print("\nRunning without selection control...")
    traj_no_pid, final_no_pid = run_justinianic(use_pid=False)

    print("Running with selection control...")
    traj_pid, final_pid = run_justinianic(use_pid=True)
