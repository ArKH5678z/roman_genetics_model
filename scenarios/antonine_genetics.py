...
# 165 CE = generation 26
START_YEAR = 165
START_GEN  = 26
DURATION   = 2      # generations of strong selection pressure
DAYS       = GENERATIONS

# Antonine hits Eastern Med hardest — originated in the East
# then spread west through Roman legions returning from Parthia
PLAGUE_EVENTS = {
    'Eastern_Med':         [(START_GEN, 0.07, DURATION)],
    'Italian_Central_Med': [(START_GEN, 0.05, DURATION)],
    'Western_European':    [(START_GEN, 0.02, DURATION)]
}
...
def run_antonine(use_pid=True, migration_rate=0.001):
    """
    Run Antonine plague simulation across three subpopulations.
...
if __name__ == "__main__":
    print("Antonine Plague Simulation (165 CE)")
    print("=" * 45)

    print("\nRunning without selection control...")
    traj_no_pid, final_no_pid = run_antonine(use_pid=False)

    print("Running with selection control...")
    traj_pid, final_pid = run_antonine(use_pid=True)
